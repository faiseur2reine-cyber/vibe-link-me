...
const ProfileEditor = ({ profile, onUpdate, onRefetch }: ProfileEditorProps) => {
  ...
  const handleSave = async () => {
    setSaving(true);
    const result = await onUpdate({ display_name: displayName, bio });
    if (result?.error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <Button onClick={handleSave} disabled={saving} className="rounded-full bg-gradient-to-r from-primary to-secondary hover:opacity-90">
